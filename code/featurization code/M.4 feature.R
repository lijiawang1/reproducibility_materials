...
set_celltype = unique(aggExprs_meta$celltype)
set_patient = unique(aggExprs_meta$rp_id)

length_cell = length(set_celltype)
avg_matrix = matrix(0,nrow = dim(scMerge)[1],ncol = length_cell)
colnames(avg_matrix) = set_celltype
rownames(avg_matrix) = rownames(scMerge)

for (i in 1:length_cell){
  cell_i = set_celltype[i]
  index_cell = which(aggExprs_meta$celltype == cell_i)
  matrix_cell =  scMerge[,index_cell]
  length_gene = rowSums(matrix_cell != 0)
  length_gene[ length_gene <= 0 ] = 1
  avg_matrix[,i] =  rowSums( matrix_cell)/ length_gene
}
...
svd_m =  prcomp(avg_matrix,rank. = 1) 
PC1 = svd_m$rotation
names(PC1) = set_celltype
length_patient = length(set_patient)
patient_col_matrix = matrix(0,nrow = dim(scMerge)[1],ncol = length_patient)
for(i in 1:length_patient){
  patient_i = set_patient[i]
  index_patient = which(aggExprs_meta$rp_id == patient_i)
  patient_cell = aggExprs_meta$celltype[index_patient]
  matrix_patient = matrix(scMerge[,index_patient], nrow = dim(scMerge)[1])
  patient_col_matrix[,i] = as.vector(matrix_patient%*%PC1[patient_cell])
}

patient_gene = t( patient_col_matrix)
colnames(patient_gene) = rownames(scMerge)

rownames(patient_gene) = set_patient

head(patient_gene[,1:10])
########################
unique(aggExprs_meta$age2)
...
var_g = colVars(patient_gene)
mean_g = colMeans(patient_gene)
rank_g = rank(-var_g/abs(mean_g))

factor_index = which(rank_g <=3000)



diagnosis_matix = data.frame(patient_gene[,factor_index ])
...
for(i in 1:50){
  ...
  mymodel <-  multinom(condition~. ,data = diagnosis_train,MaxNWts = 10000)
...
save(list_error, file = "output/logistic_m4.Rdata")
...
save(list_error, file = "output/forest_m4.Rdata")
...
save(list_error, file = "output/svm_m4.Rdata")
