#generating M.2 feature and corresponding results 
library(nnet)
library(readr)
library(resample)
...
index_remove = which(aggExprs_meta$celltype == "ILC")

aggExprs_meta = aggExprs_meta[-index_remove,]
scMerge= scMerge[,-index_remove]

set_cell= unique(aggExprs_meta$celltype)
set_patient = unique(aggExprs_meta$rp_id)




pca = prcomp(t(scMerge),rank. = 1)
PC1 = pca$x




patient_cells = matrix(0,ncol = length(set_cell), nrow = length(set_patient))

rownames(patient_cells) = set_patient
colnames(patient_cells) = set_cell



lp = length(set_patient)


for(i in 1:length(set_patient)){
  index_patient = which(aggExprs_meta$rp_id == set_patient[i])
   patent_cell = aggExprs_meta$celltype[index_patient]
  patient_cells[i,patent_cell] = as.numeric( PC1[index_patient])
}
...
diagnosis_matix  = data.frame(patient_cells)
...
save(list_error, file = "output/logistic_m2.Rdata")
...
for(i in 1:50){
  ...
  mymodel <-   randomForest(as.factor(condition)~., data=diagnosis_train , proximity=TRUE,ntree = 3000)
...
save(list_error, file = "output/forest_m2.Rdata")
...
save(list_error, file = "output/svm_m2.Rdata")
