...
cor_matrix = matrix(0,nrow = dim(scMerge)[1],ncol = length_cell)
...
for (i in 1:length_cell){
  ...
  matrix_cell =  as.matrix(scMerge[,index_cell])
  cor_matrix[,i]=  colStdevs(t(matrix_cell))
}


cor_vector = c(cor_matrix)


set_gene = rownames(scMerge)


cm_row_name =rep(c(set_celltype),each = dim(scMerge)[1] )
length_patient = length(set_patient)
patient_col_matrix = matrix(0,nrow = dim(scMerge)[1]*length_cell,ncol = length_patient)


for(i in 1:length_patient){
  patient_i = set_patient[i]
  index_patient = which(aggExprs_meta$rp_id == patient_i)
  matrix_patient = as.vector(scMerge[,index_patient])
  patient_cell = which( cm_row_name %in% aggExprs_meta$celltype[index_patient])
  patient_col_matrix[ patient_cell,i] =  matrix_patient
}
...
for(i in 1:50){
  ...
  mymodel <-  multinom(condition~. ,data = diagnosis_train,MaxNWts = 20000)
...
save(list_error, file = "output/logistic_m1.Rdata")
...
save(list_error, file = "output/forest_m1.Rdata")
...
save(list_error, file = "output/svm_m1.Rdata")
