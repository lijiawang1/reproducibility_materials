####################
#generating SIMPLE EXAMPLE similar to the results in Figure 5
library(nnet)
...
for(ii in 1:20){
  ...
  set_1 = data.frame(Y = rep(1,n1), X = rnorm(n1, 0, 1), X2 = rnorm(n1, -3, 1))
  ...
  set_1c = data.frame(Y = rep(1,nc1), X = rnorm(nc1, 0, 1), X2 = rnorm(nc1, -3, 1))
...
plot_error= c(error_1_es,error_23_es,error_non[,2],error_non[,7])
errornames = rep(c("error1","error23"),each = dim(error_non)[1])
errornames = rep(errornames,2)
method = rep(c("H-NP","ROC"),each = length(plot_error)/2)
...
plot_data$method = factor(plot_data$method, level = c("ROC","H-NP"))
...
############################
# SIMPLE EXAMPLE half violin plots similar to that in Figure 5
